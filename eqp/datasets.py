...
class MNIST:
	def __init__(self, batch_size, device, n_train=60000, n_test=10000):
		...
		dataset_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../datasets/MNIST')
		dataset_name = 'mnist.pkl.gz'
		with gzip.open(os.path.join(dataset_path, dataset_name), 'rb') as F:
...
class CIFAR10:
	def __init__(self, batch_size, device, dtype=torch.float, n_train=50000, n_test=10000):
		...
		path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../datasets/CIFAR-10')
